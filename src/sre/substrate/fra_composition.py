...
class FRAComposedReconstruction:
    ...
    def __init__(
        ...
        *,
        ...
        self._governance = governance_hooks or FACGovernanceHooks(

            component_id="sre.fra.composed"
    ...
    def run_recursive(
        ...
        *,
        ...
        drift = float((domain_result or {}).get("drift", (domain_result or {}).get("metrics", {}).get("drift", 1.0)))

        quality = float((domain_result or {}).get("quality", (domain_result or {}).get("metrics", {}).get("quality", 0.0)))
    ...
    def run(
        ...
        recursive = self.run_recursive(
            ...
            target_language, time_period, evidence_sources, max_cycles=1
        ...
        return ComposedReconstructionResult(
            ...
            should_continue=not recursive.converged and recursive.domain_result.get("status") == "COMPLETED",
    ...
    def _run_legacy_pass(
        ...
        self._governance.on_cycle_start(cycle_id, {
        ...
        should_continue = (
            ...
            status == "COMPLETED" and drift <= self._drift_threshold and quality > 0.0
    ...
    def _collect_drift_events(
        ...
        self, contexts: list[CycleContext], drift: float
                ...
                events.append(
                    ...
                    DriftEvent(
                        ...
                        message=(
                            ...
                            f"SRE drift {drift:.3f} exceeds threshold "

                            f"{self._drift_threshold}"
